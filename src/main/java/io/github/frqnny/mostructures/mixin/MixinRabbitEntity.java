...
public abstract class MixinRabbitEntity extends LivingEntity {
    ...
    public void die(DamageSource source) {
        ...
        if (thisEntity.getRabbitType() == 99) {
            ...
            ItemHelper.spawnStack(thisEntity.level, thisEntity.getX(), thisEntity.getY() + 0.5, thisEntity.getZ(), diamonds);

            ItemStack emeralds = new ItemStack(Items.EMERALD);
            emeralds.setCount(Math.max(10, random.nextInt(12)));
            ItemHelper.spawnStack(thisEntity.level, thisEntity.getX(), thisEntity.getY() + 0.5, thisEntity.getZ(), emeralds);

            ItemStack ironIngots = new ItemStack(Items.IRON_INGOT);
            ironIngots.setCount(Math.max(18, random.nextInt(20)));
            ItemHelper.spawnStack(thisEntity.level, thisEntity.getX(), thisEntity.getY() + 0.5, thisEntity.getZ(), ironIngots);
        }
